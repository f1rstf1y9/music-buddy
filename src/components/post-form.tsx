...
const MusicSearchTop = styled.div`
  display: flex;
  align-items: center;
  justify-content: space-between;
  & > .close-btn {
...
export default function PostForm() {
  ...
  const [musicResult, setMusicResult] = useState<MusicData[]>([]);
  ...
  const onMusicValueChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setMusicValue(e.target.value);
    fetch(
      `https://cors-anywhere.herokuapp.com/http://www.maniadb.com/api/search/${musicValue}/?sr=song&display=10&key=example&v=0.5`
    )
      .then((res) => res.text())
      .then((text) => {
        const data = xmlJs.xml2json(text, { compact: true, spaces: 4 });
        const musics = JSON.parse(data.replace(/maniadb:/g, "")).rss.channel
          .item;
        console.log(musics);
        setMusicResult(musics);
      });
  };
